...
serve(async (req) => {
  ...
  try {
    // SECURITY: Verify authentication - only authenticated admins can trigger alerts
    ...
    // Verify the caller has admin role via the user_roles table
    ...
    if (roleError || !roleData || (roleData.role !== 'admin' && roleData.role !== 'trainer')) {
      return new Response(
        JSON.stringify({ error: 'Forbidden - Admin or trainer role required' }),
