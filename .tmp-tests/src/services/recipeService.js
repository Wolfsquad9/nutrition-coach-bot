...
exports.getMealSuitability = getMealSuitability;
exports.generateFullDayMealPlan = generateFullDayMealPlan;
...
function getSuitableIngredients(selectedFoods, mealType) {
    return ingredientDatabase_1.coreIngredients.filter(ing => {
...
function generateInstructions(ingredients, mealType) {
    const instructions = [];
    ...
    if (mealType === 'breakfast') {
        ...
        if (carb)
            instructions.push(`Cook ${carb.name.toLowerCase()} according to package directions.`);
        if (protein)
            instructions.push(`Prepare ${protein.name.toLowerCase()} (scramble, poach, or as desired).`);
        ...
        if (protein)
            instructions.push(`Season ${protein.name.toLowerCase()} with salt, pepper, and preferred spices.`);
        if (protein)
            instructions.push(`Cook ${protein.name.toLowerCase()} in a pan over medium-high heat until done.`);
        if (carb)
            instructions.push(`Meanwhile, prepare ${carb.name.toLowerCase()} according to directions.`);
        if (vegetables.length > 0) {
...
function calculateTotalMacros(ingredients) {
    return ingredients.reduce((total, ing) => {
        const macros = (0, ingredientDatabase_1.calculateMacros)(ing, ing.typical_serving_size_g);
        return {
...
function generateRecipe(selectedFoods, mealType) {
    ...
    const recipeIngredients = selectedIngredients.map(ing => ({
        ...
        unit: 'g',
        category: ing.category === 'carbohydrate' ? 'carb' : ing.category === 'misc' ? 'spice' : ing.category,
        macrosPer100g: {
...
function determineDietTypes(ingredients) {
    const dietTypes = [];
    const hasAnimalProtein = ingredients.some(i => ['chicken-breast', 'salmon', 'turkey-breast', 'tuna'].includes(i.id));
    const hasDairy = ingredients.some(i => ['greek-yogurt', 'cottage-cheese'].includes(i.id));
    ...
    const isGlutenFree = !ingredients.some(i => ['whole-wheat-pasta', 'whole-wheat-bread', 'barley', 'oats'].includes(i.id));
    if (isGlutenFree)
        dietTypes.push('gluten-free');
    return dietTypes;
}
function determineAllergens(ingredients) {
    const allergens = [];
    if (ingredients.some(i => i.id === 'eggs'))
        allergens.push('eggs');
    if (ingredients.some(i => ['greek-yogurt', 'cottage-cheese'].includes(i.id)))
        allergens.push('dairy');
    if (ingredients.some(i => ['almonds', 'walnuts', 'peanut-butter'].includes(i.id)))
        allergens.push('nuts');
    if (ingredients.some(i => ['salmon', 'tuna'].includes(i.id)))
        allergens.push('fish');
    if (ingredients.some(i => i.id === 'tofu'))
        allergens.push('soy');
    if (ingredients.some(i => ['whole-wheat-pasta', 'whole-wheat-bread', 'barley'].includes(i.id)))
        allergens.push('gluten');
    return allergens;
}
function determineEquipment(mealType) {
    switch (mealType) {
        case 'breakfast':
...
function getMealSuitability(ingredientId) {
    const ingredient = ingredientDatabase_1.coreIngredients.find(ing => ing.id === ingredientId);
 ...
 */
function checkMacroTolerance(actual, target) {
    const calcPercentVariance = (actualVal, targetVal) => targetVal > 0 ? (actualVal - targetVal) / targetVal : 0;
 ...
 */
function adjustMealIngredients(dailyPlan, totalMacros, targetMacros, toleranceCheck, bodyweightKg) {
    const mealTypes = ['breakfast', 'lunch', 'dinner', 'snack'];
    const adjustedPlan = JSON.parse(JSON.stringify(dailyPlan));
    const adjustedMacros = { ...totalMacros };
    const constraints = (0, nutritionScience_1.createEmptyConstraints)();
    ...
    for (const macro of nutritionScience_1.MACRO_ADJUSTMENT_ORDER) {
    ...
    for (const adjustment of priorityAdjustments) {
        ...
        const targetRole = nutritionScience_1.MACRO_TO_ROLE[macro];
        ...
        for (const mealType of mealTypes) {
            ...
            for (let i = 0; i < meal.ingredients.length; i++) {
                ...
                const ingRole = (0, nutritionScience_1.calculateIngredientRole)(ing.macros);
                ...
                const enhancedIng = (0, nutritionScience_1.enhanceIngredientWithRole)(ing, bodyweightKg);
                ...
                if (newServingRaw > maxGrams) {
                    (0, nutritionScience_1.recordConstraintHit)(constraints, ing.id, ing.name, maxGrams, newServingRaw);
                }
            ...
            if (constraints.realismConstraintHit && Math.abs(remainingDeficit) > targetMacros[macro] * MACRO_TOLERANCES[macro]) {
                ...
                for (let i = 0; i < meal.ingredients.length; i++) {
                    ...
                    if (ingRole !== targetRole)
                        continue;
                    const enhancedIng = (0, nutritionScience_1.enhanceIngredientWithRole)(ing, bodyweightKg);
                    if (ing.typical_serving_size_g >= enhancedIng.maxPerMealGrams)
                        continue;
 ...
 */
function generateDeterministicRecipeName(ingredients, mealType) {
    const protein = ingredients.find(i => i.category === 'protein');
    const carb = ingredients.find(i => i.category === 'carbohydrate');
 ...
 */
function generateDeterministicInstructions(ingredients, mealType) {
    const protein = ingredients.find(i => i.category === 'protein');
    const carb = ingredients.find(i => i.category === 'carbohydrate');
    const vegetables = ingredients.filter(i => i.category === 'vegetable');
 ...
 */
function generateFullDayMealPlan(selectedFoods, macroTargets) {
    const mealTypes = ['breakfast', 'lunch', 'dinner', 'snack'];
    let dailyPlan = {
    ...
    let totalMacros = { calories: 0, protein: 0, carbs: 0, fat: 0, fiber: 0 };
    ...
    const accumulatedConstraints = (0, nutritionScience_1.createEmptyConstraints)();
    while (iteration < MAX_CONVERGENCE_ITERATIONS && !converged) {
        ...
        const currentVariance = Math.abs(toleranceCheck.percentageVariance.calories) +
        ...
        const { adjustedPlan, adjustedMacros, constraints } = adjustMealIngredients(dailyPlan, totalMacros, macroTargets, toleranceCheck);
    ...
    if (!converged) {
        const finalVariance = Math.abs(finalCheck.percentageVariance.calories) +
 ...
 */
function generateMealRecipeText(recipeName, ingredients, instructions) {
    const ingredientList = ingredients
        .map(ing => `• ${ing.name}: ${ing.typical_serving_size_g}g`)
        .join('\n');
    const instructionList = instructions
 ...
 */
function canGenerateFullDayPlan(selectedFoods) {
    const mealTypes = ['breakfast', 'lunch', 'dinner', 'snack'];
    const missingMeals = [];
    for (const mealType of mealTypes) {
        const suitableIngredients = ingredientDatabase_1.coreIngredients.filter(ing => selectedFoods.includes(ing.id) && ing.allowedMeals.includes(mealType));
 ...
 */
function shuffleForDay(selectedFoods, dayIndex) {
 ...
 */
function generateWeeklyMealPlan(selectedFoods, macroTargets) {
    const days = [];
    const weeklyTotalMacros = { calories: 0, protein: 0, carbs: 0, fat: 0, fiber: 0 };
